...
> - **Psikotes AI (Layer 2, untuk nanny) — SELESAI DIKODING 10 Juli 2026**: instrumen **Capture Work Style** (milik HCC sendiri, revisi PAPI Kostick — 90 blok forced-choice, 20 dimensi, lihat `docs/opds/19_instrumen_capture_work_style.md`) built-in di `apps/web` (deviasi terkontrol dari ADR-009, lihat [ADR-011](../../docs/opds/08_adr/ADR-011_capture-work-style-built-in.md)). Data soal: `src/lib/capture-work-style-instrument.ts`. Skoring: `src/lib/capture-work-style-scoring.ts` — hitung raw 0–9 per dimensi DAN skor 8 aspek A1-A8 (dipakai Layer 3 nanti), keduanya tersimpan lengkap di `rawScores` JSON pada `AssessmentResult`. **Kartika sempat minta tampilkan raw score + kode dimensi + istilah Inggris ke orang tua (meniru contoh laporan korporat HCC "Sikap Kerja.pdf")** — ini DIBATALKAN karena melanggar aturan lama CLAUDE.md §5 ("Istilah psikologi di konten yang dibaca orang tua — DILARANG"). **Keputusan final (10 Juli 2026): `NannyDetailDrawer` HANYA menampilkan status "✓ Hasil sudah terbuka" + pesan "ringkasan sedang disiapkan"** setelah orang tua bayar — TIDAK ada kode dimensi (N/G/K dst), TIDAK ada istilah Inggris, TIDAK ada angka mentah ke orang tua. Constant `CAPTURE_WORK_STYLE_CATEGORIES`/`DIMENSION_FULL_NAME` di `capture-work-style-scoring.ts` masih ada (dipakai untuk kebutuhan internal/laporan psikolog nanti, BUKAN untuk ditampilkan ke parent) — jangan render field itu langsung ke UI orang tua. Begitu bank narasi bahasa awam untuk 8 aspek siap, sambungkan ke `NannyDetailDrawer` menggantikan placeholder ini. **Proctoring (screen capture/hidden camera) dari contoh laporan itu SENGAJA TIDAK dibangun** — Kartika eksplisit menolak. Nanny isi gratis di `/dashboard/nanny/tes-sikap-kerja` → `AssessmentResult` (layer `LAYER_2`, `testType: "Capture Work Style"`, `interpretedBy: "AI"`). Orang tua bayar Rp300rb (`ADDON_PSIKOTES_FEE_IDR`) per nanny untuk buka status — checkout `api/payment/psikotes-addon`, unlock permanen via `MatchResult.psikotesUnlocked` (bukan model baru — reuse row yang sudah ada per pasangan parent-nanny), webhook `handlePsikotesAddonSuccess`. Helper `src/lib/psikotes.ts` (`getPsikotesInfo`) dipakai `api/matching/detail` & `api/matching/calculate` supaya `NannyDetailDrawer` selalu dapat field `psikotes` konsisten dari kedua endpoint itu.
