    ...
    <div className="max-w-[480px] mx-auto px-4 pt-5 pb-28">
      ...
      <div className="bg-white border border-[#E0D0F0] rounded-[16px] overflow-hidden mb-4">
        {modules.map((mod, i) => (
          <div
            key={i}
            className={`flex items-center gap-3 p-3.5 ${i < modules.length - 1 ? "border-b border-[#E0D0F0]" : ""}`}
          >
            ...
            <span className={`text-[11px] font-semibold border px-2.5 py-0.5 rounded-full flex-shrink-0 ${mod.statusColor}`}>
              {mod.status}
            </span>
          </div>
        ))}
