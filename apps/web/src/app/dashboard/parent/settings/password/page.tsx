  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!json.success) {
        ...
        setSuccess(true)
        setTimeout(() => router.push("/dashboard/parent/settings"), 1500)
      }
    } catch {
              ...
              return (
                <div key={label}>
                  ...
                  <input
                    ...
                    required
                    minLength={i > 0 ? 8 : 1}
                    className="w-full px-3.5 py-2.5 text-[14px] text-[#5A3A7A] bg-white border-[1.5px] border-[#C8B8DC] rounded-[10px] min-h-[48px] focus:border-[#5BBFB0] focus:ring-2 focus:ring-[#5BBFB0]/15 outline-none transition-all"
                  />
                </div>
              )
            }
