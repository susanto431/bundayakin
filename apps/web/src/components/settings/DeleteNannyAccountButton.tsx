...
export function DeleteNannyAccountButton() {
  const [step, setStep] = useState<"idle" | "confirm" | "loading">("idle")

  async function handleDelete() {
    setStep("loading")
    try {
      await fetch("/api/nanny/profile", { method: "DELETE" })
    } catch {
      // best-effort
    }
  ...
  return (
    <button
      ...
      className="block py-2.5 text-[#C75D5D] font-semibold text-[13px] min-h-[40px] disabled:opacity-50"
    >
