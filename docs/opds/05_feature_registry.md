...
| Psikotes AI Layer 2 (Capture Work Style) | ✅ | 1 | **Live di production sejak 11 Juli 2026** (deploy otomatis Vercel dari push ke main, terverifikasi via `bundayakin.com/dashboard/nanny/tes-sikap-kerja`). Instrumen Capture Work Style (90 blok, 20 dimensi) built-in di `apps/web` — deviasi terkontrol dari ADR-009, lihat [ADR-011](08_adr/ADR-011_capture-work-style-built-in.md) & [19_instrumen_capture_work_style.md](19_instrumen_capture_work_style.md). Data soal `src/lib/capture-work-style-instrument.ts`, skoring `src/lib/capture-work-style-scoring.ts`. Nanny isi gratis di `/dashboard/nanny/tes-sikap-kerja`. Orang tua buka detail per nanny Rp300rb (`ADDON_PSIKOTES_FEE_IDR`) via `api/payment/psikotes-addon`, unlock permanen di `MatchResult.psikotesUnlocked`, tampil di `NannyDetailDrawer` |
| Review psikolog Layer 3 (Nanny Care Profile™) | 📋 | 1 | Schema ready (`ADDON_PSIKOLOG`/`AssessmentResult`). Keputusan produk lengkap (10 Juli 2026): on-demand per nanny (orang tua bayar Rp1,2jt flat, termasuk interview + 1 sesi konsultasi hasil), bukan wajib semua nanny. SOP: Tester (Admin HCC) pandu sesi Zoom tes grafis (DAP+BAUM) + upload manual, psikolog baca grafis & tulis catatan di atas skor Layer 2 (**Layer 2 sudah selesai dikoding — prasyarat terpenuhi**). PDF via `apps/pdf-service` (ReportLab) mengikuti pola NannyProfile report. Psikolog pengerjanya pakai akun `PsikologProfile` yang sama dengan Konsultasi Psikolog Anak, dibedakan lewat penanda jenis layanan (belum diimplementasi). Belum dikoding |
...
| Add-on psikotes payment | ✅ | 1 | **Sudah dikoding & live sejak 10 Juli 2026** (baris sebelumnya ketinggalan zaman, ditulis sebelum fitur dibangun). Checkout Mayar via `api/payment/psikotes-addon`, lihat baris "Psikotes AI Layer 2" di atas |
| Undangan Psikotes | ✅ | 1 | **Selesai dikoding 12 Juli 2026.** Checkout Mayar via `api/payment/psikotes-invite` (satu endpoint, dua jalur): nanny sudah match/terdaftar → tombol di `NannyDetailDrawer`, hasil otomatis terbuka begitu dia selesai (`handlePsikotesInviteSuccess`); nanny belum terdaftar sama sekali (`PsikotesInviteForm` di `dashboard/parent/matching`, `PSIKOTES_INVITE_ENABLED = true`) → webhook langsung buat akun Nanny mandiri (bukan Flow A, lihat [ADR-017](08_adr/ADR-017_undangan-psikotes-nanny-belum-terdaftar.md)), aktivasi via WA OTP (`/auth/forgot-password?phone=`) langsung ke halaman psikotes (`psikotesOnlyOnboarding` di `NannyProfile`). Hasil nanny off-platform dikirim ke parent via WA+email (`lib/emails/psikotes-invite-result.ts`), belum ada halaman riwayat undangan di dashboard (scope v1). Pengingat WA harian via cron `api/cron/psikotes-reminders` (`vercel.json`, perlu `CRON_SECRET`) |
...
| Skrining Perkembangan (KPSP) | 🔄 | 2 | **Selesai dikoding Juli 2026, menunggu deploy.** Form per kelompok usia (Ya/Tidak + alat bantu visual sederhana), hasil & rekomendasi (khusus pelanggan, catat gratis), riwayat, kartu navigasi baru di profil anak. Booking Konsultasi Psikolog Anak terhubung langsung dari kartu hasil (jembatan WA lama sudah diganti) — sejak 11 Juli 2026 ditawarkan sebagai soft-selling di **semua tingkat hasil**, bukan cuma "Sebaiknya konsultasi", termasuk untuk akun belum-langganan. Fix bug: guard submit ganda (`submittingRef`) mencegah 2 record tersimpan dari 1 pengisian |
| Konsultasi Psikolog Anak (add-on/sesi) | 🔄 | 2 | **Selesai dikoding Juli 2026, menunggu deploy.** Checkout otomatis via Mayar, harga lewat Pricing Config Panel (4 key: Junior/Mid/Senior/Pelanggan — hanya Mid & Pelanggan yang aktif dijual saat peluncuran). Booking pilih tanggal + Slot Konsultasi tetap (09:00/13:00/16:00), sistem assign otomatis ke psikolog **Mid** aktif sesuai kapasitas harian (dikoreksi 11 Juli 2026 — kode sempat salah hardcode ke Senior, lihat ADR-012) |
| Portal Psikolog (role baru) | 🔄 | 2 | **Selesai dikoding Juli 2026, menunggu deploy.** Role `PSIKOLOG` baru, akun dibuat manual admin (`/dashboard/admin/psikolog`, lihat ADR-010). Dashboard psikolog (`/dashboard/psikolog`): antrean sesi terjadwal/selesai + catatan hasil sesi. Kapasitas 3/hari/psikolog (maks 5, per profil). Antrean review konten Edukasi Terkurasi menyusul Tahap 3 |
| Pilih psikolog spesifik + kalender warna (booking) | 📋 | 2 | **Belum dikoding** (keputusan 11 Juli 2026, lihat [ADR-012](08_adr/ADR-012_pilih-psikolog-dan-jadwal-individual.md) & [PRD §7c](13_prd_tumbuh_kembang.md)). Dua pintu masuk: pilih psikolog dulu (profil + Jam Terbang Psikolog + tanda pernah menangani anak ini) atau pilih tanggal dulu (lihat psikolog tersedia). Tampilan kalender booking juga diubah dari klik-per-tanggal jadi tampilan bulan/minggu berwarna (kosong/penuh) |
| Jadwal Psikolog individual (Portal Psikolog) | 📋 | 2 | **Belum dikoding** (ADR-012). Psikolog atur pola mingguan berulang (on/off per hari dari 3 Slot Konsultasi baku) + tandai cuti, lewat Portal Psikolog — sebelumnya jadwal psikolog tidak bisa diatur sendiri |
| Ulasan Psikolog (internal) | 📋 | 2 | **Belum dikoding** (ADR-012). Rating + pilihan ganda + esai dari orang tua setelah sesi selesai, opsional dengan ajakan halus. Hanya untuk pemantauan kualitas internal HCC — tidak pernah tampil ke orang tua lain |
