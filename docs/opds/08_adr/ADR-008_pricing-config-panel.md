...
- **Ditemukan saat implementasi, diperbaiki di commit yang sama (bukan bug baru dari fitur ini):** halaman `/pricing` publik sebelumnya menampilkan dua tingkat placement fee (Jangka Panjang Rp 1,2jt vs Temporer Rp 600rb), padahal kode pembayaran (`api/payment/placement`) hanya pernah mengenakan **satu** biaya flat — tidak ada pembedaan berdasar `nannyType`. Keputusan Kartika: tidak perlu membangun pembedaan harga; salinan marketing diperbarui menjadi satu angka "Biaya Penempatan" saja, konsisten dengan yang benar-benar di-charge.
- **Bug ditemukan & diperbaiki 13 Juli 2026:** nilai efektif dihitung dengan `orderBy effectiveFrom desc` saja. Karena form penjadwalan pakai `<input type="date">` (tanpa jam), beberapa perubahan yang dijadwalkan di tanggal yang sama menghasilkan `effectiveFrom` yang persis identik (tengah malam) — untuk baris yang timestamp-nya sama, urutan hasil query database tidak terjamin konsisten, jadi harga yang tampil sebagai "berlaku sekarang" bisa jadi bukan perubahan terakhir yang dibuat admin. Fix: tambah urutan kedua `createdAt desc` di `fetchEffectiveValues` dan `listScheduleForKey` (`src/lib/pricing-config.ts`) — perubahan yang paling terakhir dibuat sekarang selalu yang menang saat `effectiveFrom` sama.
